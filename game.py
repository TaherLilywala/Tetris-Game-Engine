...
DISPLAY_NEXT_PIECE = True #Display NEXTQ
...
class MainBoard:
	...
	def draw_BLOCK(self,gameDisplay,xRef,yRef,row,col,color):
	...
	def draw_GAMEBOARD_BORDER(self,gameDisplay):
	...
	def draw_GAMEBOARD_CONTENT(self,gameDisplay):
			...
			for row in range(0,self.rowNum):
				for col in range(0,self.colNum):
					if self.blockMat[row][col] == 'empty':
						self.erase_BLOCK(gameDisplay,self.xPos,self.yPos,row,col)
					else:
						self.draw_BLOCK(gameDisplay,self.xPos,self.yPos,row,col,blockColors[self.blockMat[row][col]])
						
			if self.piece.status == 'moving':
				for i in range(0,4):
					self.draw_BLOCK(gameDisplay,self.xPos,self.yPos,self.piece.blocks[i].currentPos.row, self.piece.blocks[i].currentPos.col, blockColors[self.piece.type])
	...
	def draw_SCOREBOARD_BORDER(self,gameDisplay):
	...
	def draw_SCOREBOARD_CONTENT(self,gameDisplay):
		...
		if self.gameStatus == 'running':
			if DISPLAY_NEXT_PIECE == True:
				nextPieceText = fontSB.render('next:', False, TEXT_COLOR)
			else:
				nextPieceText = fontSB.render('', False, TEXT_COLOR)
			...
			for i in range(0,4):
				...
				if DISPLAY_NEXT_PIECE == True:
					if self.nextPieces[1] == 'O':
						self.draw_BLOCK(gameDisplay,xPosRef+0.5*self.blockSize,yPosRef+2.25*self.blockSize,blocks[i][ROW],blocks[i][COL],blockColors[self.nextPieces[1]])
					elif self.nextPieces[1] == 'I':
						self.draw_BLOCK(gameDisplay,xPosRef+0.5*self.blockSize,yPosRef+1.65*self.blockSize,blocks[i][ROW],blocks[i][COL],blockColors[self.nextPieces[1]])
					else:
						self.draw_BLOCK(gameDisplay,xPosRef+1*self.blockSize,yPosRef+2.25*self.blockSize,blocks[i][ROW],blocks[i][COL],blockColors[self.nextPieces[1]])
	...
	def draw(self,gameDisplay):
		...
		self.draw_GAMEBOARD_BORDER(gameDisplay)
		self.draw_SCOREBOARD_BORDER(gameDisplay)
		
		self.draw_GAMEBOARD_CONTENT(gameDisplay)
		self.draw_SCOREBOARD_CONTENT(gameDisplay)
...
class Game:
	...
	def gameLoop(self, gameDisplay, clock, gameClock):		
		...
		while not gameExit: #Stay in this loop unless the game is quit
			...
			mainBoard.draw(gameDisplay) #Draw the new board after game the new game actions
